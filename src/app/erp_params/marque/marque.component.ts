...
import { MarqueCreateUpdateComponent } from './marque-create-update/marque-create-update.component';
...
export class MarqueComponent {
   ...
   */
  subject$: ReplaySubject<Marque[]> = new ReplaySubject<Marque[]>(1);
  data$: Observable<Marque[]> = this.subject$.asObservable();
  listItems: Marque[] = [];

  @Input()
  columns: TableColumn<Marque>[] = [
    {
      label: 'LIBELLE',
      property: 'libelle',
      type: 'text',
  ...
  selection = new SelectionModel<Marque>(true, []);
  ...
  constructor(private service:MarqueService, private serviceHttp:MarqueHttpService, private dialog: MatDialog, private router:Router) {}
  ...
  ngOnInit() {
    ...
    this.data$.pipe(filter<Marque[]>(Boolean)).subscribe((listItems) => {
  ...
  create() {
    ...
    .afterClosed()
    .subscribe((item: Marque) => {
      if (item) {
        this.listItems.unshift(new Marque(item));
  ...
  update(item: Marque) {
    ...
    .subscribe((updatedItem) => {
      if (updatedItem) {
        ...
        this.listItems[index] = new Marque(updatedItem);
        this.subject$.next(this.listItems);
      }
    });
  }
