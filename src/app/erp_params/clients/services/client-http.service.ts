...
export class ClientHttpService {
  ...
  REST_API = environment.baseUrl+'/client';
  ...
  GetAllTypeContact(): Observable<any> {
    let data = {code_societe:  this.tokenService.getCodeSociete()}
    return this.httpClient.post(`${environment.baseUrl+"/typecontact/all?t=1"}`,data , this.tokenService.getHeader())
    .pipe(catchError(this.tokenService.handleErrorWithParams()));
  }
  ...
  GetAllTiersCategorie(): Observable<any> {
    let data = {code_societe:  this.tokenService.getCodeSociete()}
    return this.httpClient.post(`${environment.baseUrl+"/tierscategorie/all?t=1"}`,data , this.tokenService.getHeader())
    .pipe(catchError(this.tokenService.handleErrorWithParams()));
  }
