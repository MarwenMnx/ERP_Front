...
import people from './sample-data';
import data from '../../document-vente-create-update/fakedata';
import { LigneDocumentAchat } from 'src/app/erp_documents_achat/models/document-achat.model';
...
@Component({
  selector: 'vex-liste-ligne-accordion',
  templateUrl: './liste-ligne-accordion.component.html',
  styleUrls: ['./liste-ligne-accordion.component.scss'],
  standalone: true,
  imports: [  
    SharedModule,
...
export class ListeLigneAccordionComponent {

  displayedColumns: string[] = ['numeroDateDoc', 'quantiteUnite1', 'quantiteUnite2'];
  public dataSource = new MatTableDataSource<LigneDocumentVente | Group>([]);
  ...
  columnsAchat: TableColumn<LigneDocumentAchat>[] = [
    {
      label: 'Numéro',
      property: 'numero',
      type: 'text',
  ...
  async ngOnInit() {
    if(this.isAchat){
      this.columns = this.columnsAchat
    }
  }
  ...
  customFilterPredicate(data: LigneDocumentVente | Group, filter: string): boolean {
    return (data instanceof Group) ? data.visible : this.getDataRowVisible(data);
  }

  getDataRowVisible(data: LigneDocumentVente): boolean {
