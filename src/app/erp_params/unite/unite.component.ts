...
import { UniteCreateUpdateComponent } from './unite-create-update/unite-create-update.component';
...
export class UniteComponent {
   ...
   */
  subject$: ReplaySubject<Unite[]> = new ReplaySubject<Unite[]>(1);
  data$: Observable<Unite[]> = this.subject$.asObservable();
  listItems: Unite[] = [];

  @Input()
  columns: TableColumn<Unite>[] = [
    {
      label: 'LIBELLE',
      property: 'libelle',
      type: 'text',
  ...
  selection = new SelectionModel<Unite>(true, []);
  ...
  constructor(private service:UniteService, private serviceHttp:UniteHttpService, private dialog: MatDialog, private router:Router) {}
  ...
  ngOnInit() {
    ...
    this.data$.pipe(filter<Unite[]>(Boolean)).subscribe((listItems) => {
  ...
  create() {
    ...
    .afterClosed()
    .subscribe((item: Unite) => {
      if (item) {
        this.listItems.unshift(new Unite(item));
  ...
  update(item: Unite) {
    ...
    .subscribe((updatedItem) => {
      if (updatedItem) {
        ...
        this.listItems[index] = new Unite(updatedItem);
        this.subject$.next(this.listItems);
      }
    });
  }
