...
import { retourMService } from '../../societe-service.service';
...
@Component({
  selector: 'vex-mretour',
  templateUrl: './mretour.component.html',
  styleUrls: ['./mretour.component.scss'],
  standalone: true,
...
export class MretourComponent implements OnInit {
  ...
  constructor(private retourMService: retourMService) { }
  ...
  fetchData(): void {
    this.retourMService.getAllretourM().pipe(
  ...
  createChart(data: ResultatItem[]): void {
    ...
    this.achatChart = new Chart('MarChart', {
      type: this.chartType,
      data: {
        labels: labels,
        datasets: [
          {
            label: `Nombre de Bons retour marchandise - ${this.selectedSociete}`,
            data: counts,
            backgroundColor: 'rgba(128, 0, 128, 0.2)', // Prune
            borderColor: 'rgba(128, 0, 128, 1)', // Prune
            borderWidth: 1,
          ...
          },
          {
            ...
            data: totalTTCs,
            backgroundColor: 'rgba(255, 20, 147, 0.2)', // Rose vif
      ...
      options: {
        ...
        scales: {
          'y-axis-count': {
            ...
            title: {
              display: true,
              text: 'Nombre de Bons retour marchandise'
            },
            grid: {
  ...
  generateCSV(): void {
    ...
    csvContent += 'Code Société,Nombre de Bons retour marchandise,Total TTC,Month,Year\n'; // Ajoutez les en-têtes
  ...
  initializeEmptyChart(): void {
    ...
    this.achatChart = new Chart('MarChart', {
      ...
      data: {
        ...
        datasets: [
          {
            label: 'Nombre de Bons retour marchandise',
            data: [],
            backgroundColor: 'rgba(128, 0, 128, 0.2)', // Prune
            borderColor: 'rgba(128, 0, 128, 1)', // Prune
            borderWidth: 1,
