...
export class TransporteurHttpService{
  ...
  // Add
  AddNew(data: Transporteur): Observable<any> {
  ...
  // Update
  update(data: Transporteur): Observable<any> {
    let newdata:any = data
    newdata.code_societe = this.tokenService.getCodeSociete()
  ...
  getData(items:any) {
    ...
    //   newItems.push(new Transporteur(items[key]))
    // }
    return items
  }
}
