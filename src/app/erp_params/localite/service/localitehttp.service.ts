...
export class LocalitehttpService {

  REST_API = environment.baseUrl+'/localite';

  // Http Header
  constructor(private httpClient: HttpClient, private tokenService:TokenService) {
  }
  //get all typeDepartement
  ...
  // Add
  AddNew(data: Localite): Observable<any> {
  ...
  // Update
  update(data: Localite): Observable<any> {
    let newdata:any = data
    newdata.code_societe = this.tokenService.getCodeSociete()
  ...
  getData(items:any) {
    ...
    //   newItems.push(new Localite(items[key]))
    // }
    return items
  }
}
