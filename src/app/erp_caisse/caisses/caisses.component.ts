...
import {CaissesCreateUpdateComponent} from "./caisses-create-update/caisses-create-update.component";


@Component({
  selector: 'vex-caisses',
...
export class CaissesComponent {
   ...
   */
  subject$: ReplaySubject<Caisses[]> = new ReplaySubject<Caisses[]>(1);
  data$: Observable<Caisses[]> = this.subject$.asObservable();
  listItems: Caisses[] = [];

  @Input()
  columns: TableColumn<Caisses>[] = [
    {
      label: 'NUMERO',
      property: 'numero',
      type: 'text',
    ...
    {
      label: 'utilisateur_ouverture_caisse',
      property: 'utilisateur_ouverture_caisse',
      type: 'text',
      visible: true,
      cssClasses: ['font-medium']
    },
    {
      label: 'utilisateur_cloture_caisse',
      property: 'utilisateur_cloture_caisse',
      type: 'text',
      visible: true,
      cssClasses: ['font-medium']
    },
  ...
  dataSource!: MatTableDataSource<Caisses>;
  selection = new SelectionModel<Caisses>(true, []);
  ...
  constructor(private serviceHttp: CaissesHttpService,public utilService: UtilService,
  ...
  ngOnInit() {
    ...
    this.data$.pipe(filter<Caisses[]>(Boolean)).subscribe((listItems) => {
  ...
  create() {
    this.dialog
      .open(CaissesCreateUpdateComponent, {})
      .afterClosed()
      .subscribe((item: Caisses) => {
        if (item) {
          this.listItems.unshift(new Caisses(item));
    ...
    this.dialog
      ...
      .subscribe((updatedItem) => {
        if (updatedItem) {
          ...
          this.listItems[index] = new Caisses(updatedItem);
          this.subject$.next(this.listItems);
        }
      });
  }
