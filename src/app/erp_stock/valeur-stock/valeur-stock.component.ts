...
import {ValeurStockHttpService} from "./services/valeur-stock-http.service";
import {Valeur_stock} from "./models/valeur-stock";
...
@Component({
  selector: 'vex-valeur-stock',
  templateUrl: './valeur-stock.component.html',
  styleUrls: ['./valeur-stock.component.scss'],
...
export class ValeurStockComponent  {
   ...
   */
  subject$: ReplaySubject<Valeur_stock[]> = new ReplaySubject<Valeur_stock[]>(1);
  data$: Observable<Valeur_stock[]> = this.subject$.asObservable();
  commandes: Valeur_stock[] = [];

  @Input()
  columns: TableColumn<Valeur_stock>[] = [
    // {
    //   label: 'Checkbox',
    //   property: 'checkbox',
    //   type: 'checkbox',
  ...
  dataSource!: MatTableDataSource<Valeur_stock>;
  selection = new SelectionModel<Valeur_stock>(true, []);
  ...
  constructor(private dialog: MatDialog, public utilService: UtilService, private serviceHttDepot: DepotHttpService,
              public utileService:UtilService, private valeurStockHttpService: ValeurStockHttpService, private tokenService: TokenService,
  ...
  get_listMvmt() {
    ...
    showLoading()
    this.valeurStockHttpService.GetAll(listFiltre).subscribe((res) => {
      //this.subject$.next(this.mouvementStockHttpService.getData(res.RESULTAT));
      //this.dataSource = new MatTableDataSource();
      this.dataSource.data = res.RESULTAT as Valeur_stock[] //this.rappelStockHttpService.getData(res.RESULTAT);
      this.show_paginate()
      hideLoading()
    });
  ...
  ngOnInit() {
    ...
    this.dataSource = new MatTableDataSource();
    this.data$.pipe(filter<Valeur_stock[]>(Boolean)).subscribe((listItems) => {
  ...
  toggleColumnVisibility(column: TableColumn<Valeur_stock>, event: Event) {
  ...
  ExportTOExcel(){
    this.impressionPdfService.ExportTOExcel(this.table, this.columns, "Valeur-stock");
  }

  exportToPdf(): void {
    this.impressionPdfService.exportToPdf(this.table, this.columns, "Valeur-stock");
  }
}
