...
export class MarqueHttpService {

  REST_API = environment.baseUrl+'/marquevehicule';

  // Http Header
  constructor(private httpClient: HttpClient, private tokenService:TokenService) {
  }
  // Add
  AddNew(data: Marque): Observable<any> {
  ...
  // Update
  update(data: Marque): Observable<any> {
    let newdata:any = data
    newdata.code_societe = this.tokenService.getCodeSociete()
  ...
  getData(items:any) {
    ...
    //   newItems.push(new Marque(items[key]))
    // }
    return items
  }
}
