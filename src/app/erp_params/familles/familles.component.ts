...
import { FamilleCreateUpdateComponent } from './famille-create-update/famille-create-update.component';
...
export class FamillesComponent {
   ...
   */
  subject$: ReplaySubject<Famille[]> = new ReplaySubject<Famille[]>(1);
  data$: Observable<Famille[]> = this.subject$.asObservable();
  listItems: Famille[] = [];

  @Input()
  columns: TableColumn<Famille>[] = [
  ...
  selection = new SelectionModel<Famille>(true, []);
  ...
  constructor(private service:FamilleService, private serviceHttp:FamilleHttpService, private dialog: MatDialog, private router:Router) {}
  ...
  ngOnInit() {
    ...
    this.data$.pipe(filter<Famille[]>(Boolean)).subscribe((listItems) => {
  ...
  create() {
    ...
    .afterClosed()
    .subscribe((item: Famille) => {
      if (item) {
        this.listItems.unshift(new Famille(item));
    ...
    this.dialog
      ...
      .subscribe((updatedItem) => {
        if (updatedItem) {
          ...
          this.listItems[index] = new Famille(updatedItem);
          this.subject$.next(this.listItems);
        }
      });
  }
  ...
  toggleColumnVisibility(column: TableColumn<Famille>, event: Event) {
