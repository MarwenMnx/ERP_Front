...
export class TypevehiculeHttpService {

  REST_API = environment.baseUrl+'/typevehicule';

  // Http Header
  constructor(private httpClient: HttpClient, private tokenService:TokenService) {
  }
  // Add
  AddNew(data: TypeVehicule): Observable<any> {
  ...
  // Update
  update(data: TypeVehicule): Observable<any> {
    let newdata:any = data
    newdata.code_societe = this.tokenService.getCodeSociete()
  ...
  getData(items:any) {
    ...
    //   newItems.push(new TypeVehicule(items[key]))
    // }
    return items
  }
}
