...
import {ReponseList} from "../../transporteur/models/transporteur.model";
import {showAlertError} from "../../../global-functions";

@Injectable({
  providedIn: 'root'
})
export class ChauffeurHttpService {

  REST_API = environment.baseUrl+'/chauffeur';

  // Http Header
  constructor(private httpClient: HttpClient, private tokenService:TokenService) {
  }
  // Add
  AddNew(data: Chauffeur): Observable<any> {
  ...
  // Update
  update(data: Chauffeur): Observable<any> {
    let newdata:any = data
    newdata.code_societe = this.tokenService.getCodeSociete()
  ...
  getData(items:any) {
    ...
    //   newItems.push(new Chauffeur(items[key]))
    // }
    return items
  }
