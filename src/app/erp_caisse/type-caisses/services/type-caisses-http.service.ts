...
export class TypeCaissesHttpService {

  REST_API = environment.baseUrl+'/typecaisses';

  // Http Header
  constructor(private httpClient: HttpClient, private tokenService:TokenService) {
  }
  // Add
  AddNew(data: Type_caisses): Observable<any> {
  ...
  // Update
  update(data: Type_caisses): Observable<any> {
    let newdata:any = data
    newdata.code_societe = this.tokenService.getCodeSociete()
  ...
  getData(items:any) {
    ...
    //   newItems.push(new Type_caisses(items[key]))
    // }
    return items
  }
}
