...
import {Caisses, ReponseList} from "../models/caisses.model";
import {showAlertError} from "../../../global-functions";

@Injectable({
  providedIn: 'root'
})
export class CaissesHttpService {

  REST_API = environment.baseUrl+'/caisse';
  // Http Header
  constructor(private httpClient: HttpClient, private tokenService:TokenService) {
  }

  AddNew(data: Caisses): Observable<any> {
 ...
 // Update
  update(data: Caisses): Observable<any> {
    let newdata:any = data
    newdata.code_societe = this.tokenService.getCodeSociete()
