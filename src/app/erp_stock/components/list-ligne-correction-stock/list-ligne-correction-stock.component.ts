...
@Component({
  selector: 'vex-list-ligne-correction-stock',
  templateUrl: './list-ligne-correction-stock.component.html',
  styleUrls: ['./list-ligne-correction-stock.component.scss'],
  standalone: true,
...
export class ListLigneCorrectionStockComponent {
  dataSource!: MatTableDataSource<LigneCorrectionStock>;
   ...
   */
  subject$: ReplaySubject<LigneCorrectionStock[]> = new ReplaySubject<LigneCorrectionStock[]>(1);
  data$: Observable<LigneCorrectionStock[]> = this.subject$.asObservable();
  ...
  columns: TableColumn<LigneCorrectionStock>[] = [
    ...
    {
      label: 'QTE1_nouvelle',
      property: 'quantite_nouvelle',
      type: 'image',
      visible: true,
      cssClasses: ['font-medium']
    },
    {
      label: 'QTE1_difference',
  ...
  selection = new SelectionModel<LigneCorrectionStock>(true, []);
  ...
  ngOnInit() {
    ...
    this.data$.pipe(filter<LigneCorrectionStock[]>(Boolean)).subscribe((lignes) => {
      this.lignes = lignes;
  ...
  updateEmplacementTable(product: LigneCorrectionStock) {

  }

  deleteProduct(item: LigneCorrectionStock) {
    this.lignes.splice(
  ...
  deleteProducts(lignes: LigneCorrectionStock[]) {
  ...
  toggleColumnVisibility(column: TableColumn<LigneCorrectionStock>, event: Event) {
  ...
  changeQuantite1(item: LigneCorrectionStock){
    item = this.ligneCorrectionStockService.changeQuantite1(item)
  }

  changeQuantite2(item: LigneCorrectionStock){
    item = this.ligneCorrectionStockService.changeQuantite2(item)
  }

  changeNouvelleQuantite(item: LigneCorrectionStock){
    item = this.ligneCorrectionStockService.changeNouvelleQte(item)
  }

  checkQuantite(item:LigneDocumentCasse){
    //return this.ligneDocumentCasseService.checkQuantiteIsValide(this.documentOriginal, this.lignes, this.allArticles, undefined, item)
  }
