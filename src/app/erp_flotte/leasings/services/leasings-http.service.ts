...
export class LeasingsHttpService{

  REST_API = environment.baseUrl+'/leasing';

  // Http Header
  constructor(private httpClient: HttpClient, private tokenService:TokenService) {
  }
  // Add
  AddNew(data: Leasings): Observable<any> {
  ...
  // Update
  update(data: Leasings): Observable<any> {
    let newdata:any = data
    newdata.code_societe = this.tokenService.getCodeSociete()
  ...
  getData(items:any) {
    ...
    //   newItems.push(new Leasings(items[key]))
    // }
    return items
  }
}
