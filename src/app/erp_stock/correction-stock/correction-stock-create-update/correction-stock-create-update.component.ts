...
import { CorrectionStockService } from '../services/correction-stock.service';
import { LigneDocumentCasseService } from '../../services/ligne-document-casse.service';
import { LigneCorrectionStockComponent } from '../../components/ligne-correction-stock/ligne-correction-stock.component';
import {MatTableDataSource} from "@angular/material/table";
@Component({
  selector: 'vex-correction-stock-create-update',
  templateUrl: './correction-stock-create-update.component.html',
  styleUrls: ['./correction-stock-create-update.component.scss'],
  standalone: true,
...
export class CorrectionStockCreateUpdateComponent {
 ...
 public defaults: DocumentCorrectionStock = new DocumentCorrectionStock(null)
 ...
 constructor(
   ...
   private service:CorrectionStockService,
   private route: ActivatedRoute,
   private UtilisateursserviceHttp:UsersHttpService,
   private ligneDocumentCorrectionStockService:LigneDocumentCasseService,
 ...
 ngOnInit() {
   if(this.id){
     ...
     this.serviceHttp.GetDetails(this.id).subscribe((res) => {
       console.log("*****res11111111111111*******")
       console.log(res)
       console.log("*****res22222222222222*******")
       //es.RESULTAT = res.RESULTAT[0]
       ...
       res.RESULTAT.lignes.forEach((res:any) => {
         res.numero = this.ligneDocumentCorrectionStockService.getLastNumero(lignes);
       });
     ...
     let user:any = this.tokenService.user
     user.societes = undefined
 ...
 save() {
   ...
   //if(!this.ligneDocumentCorrectionStockService.checkAllLigne(this.defaults, this.form.value.lignes, this.allArticles)) return
   if (!this.form.valid){
 ...
 createProduct() {
  ...
  item.lignes = item.lignes.filter((x:any) => x.quantite_difference != 0)
