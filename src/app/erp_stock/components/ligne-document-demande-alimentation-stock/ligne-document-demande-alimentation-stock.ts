...
@Component({
  selector: 'vex-ligne-document-demande-alimenatation-stock',
  templateUrl: './vex-ligne-document-demande-alimenatation-stock.component.html',
  styleUrls: ['./vex-ligne-document-demande-alimenatation-stock.component.scss'],
  standalone: true,
  ...
  imports: [
    ...
    TestFormComponent,
    LigneDocumenDemandeAlimentationStockComponent
  ]
})
export class LigneDocumenDemandeAlimentationStockComponent {
...
if (this.form.contains(newValue[0])) {
  ...
  if(!newValue[1]){
    ...
    this.form.patchValue(this.ligneDocumentDemandeAlimentationService.changeReadonlyBettweenQte1AndQte2(true, this.form.value))
    this.changeTotals()
  }
}
InputNumberChangeObservibalService.submitChangeInput()
  ...
  changeQuantite1(){
    this.form.patchValue(this.ligneDocumentDemandeAlimentationService.changeQuantite1(this.form.value))
    this.changeTotals()
    this.checkQuantite()
  }
  
  changeQuantite2(){
    this.form.patchValue(this.ligneDocumentDemandeAlimentationService.changeQuantite2(this.form.value))
    this.changeTotals()
    this.checkQuantite()
  }

  checkQuantite(){
    return this.ligneDocumentDemandeAlimentationService.checkQuantiteIsValide(this.documentOriginal, this.lignes, this.allArticles, this.form.value, undefined)
  }
  addNewLigne(){
    ...
    this.form.patchValue({
      numero:this.ligneDocumentDemandeAlimentationService.getLastNumero(this.lignes), 
  ...
  changeTotals(){
    if(this.form.controls["isQte1"].value == true){
      this.form.patchValue(this.ligneDocumentDemandeAlimentationService.changeTotals(this.form.value))
    }else{
      this.form.patchValue(this.ligneDocumentDemandeAlimentationService.changeTotals(this.form.value, true))
    }
  }
